...
if choice == "Machine Learning":
    ...
    if os.path.exists("data.csv"):
        ...
        if problema == "Classificação":
            modelo = st.selectbox("Selecione o algoritmo:", ("Logistic Regression", "Random Forest", "XGBoost"))
        if problema == "Regressão":
            modelo = st.selectbox("Selecione o algoritmo:", ("Linear Regression", "Random Forest", "XGBoost"))
            if modelo == "Linear Regression":
                st.warning("AVISO: Este algoritmo não é adequado para o problema de classificação!")
                # selectbox para selecionar o alvo
                alvo = st.selectbox("Selecione a variável alvo: ", dataframe.columns)
    ...
    if os.path.exists("model.pkl"):
        ...
        if file_pred is not None:
            ...
            if st.button:
                ...
                dataframe_pred['PROBABILIDADE DE EVASÃO (%)'] = modelo.predict_proba(dataframe_pred)[:, 0]*100
                # Visualização do conjunto de dados previsto filtrado com a probabilidade de evasão em ordem decrescente
                st.subheader("Visualização do conjunto de dados previsto")
                st.write(dataframe_pred.sort_values(by=['PROBABILIDADE DE EVASÃO (%)'], ascending=False))
